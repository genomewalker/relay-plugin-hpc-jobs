...
Read-only Slurm job status, resource usage and job logs.
...
- [ ] Define bounded job/status/log operations through Relay’s remote broker.
- [ ] Implement on-demand refresh, hidden-panel backoff and cancellation.
- [ ] Keep job submission and cancellation behind separate explicit grants.
- [ ] Test large queues, missing Slurm, permissions, old workers and dropped SSH connections.
